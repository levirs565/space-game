...
void saveScoreList(DF::Object& object) {
  ...
  if (!stream.is_open()) throw std::runtime_error("cannot save score list");
...
void ScoreListManager::addScore(const std::string& name, int score) {
  ...
  DF::Object currentScoreList = savedScore.has_value() ? std::move(savedScore.value()) : std::move(createBlankScoreList());

  DF::Object scoreObject;
  scoreObject.field["name"] = std::move(std::make_unique<DF::String>(name));
  scoreObject.field["score"] = std::move(std::make_unique<DF::String>(std::to_string(score)));

  auto* scoreListObject = currentScoreList.getField<DF::Object>("scoreList");

  scoreListObject->builder.emplace_back(
      "Score", std::move(scoreObject)
  );

  std::sort(scoreListObject->builder.begin(),
            scoreListObject->builder.end(),
            [](auto& a, auto& b) {
              int aScore = std::stoi(a.second.template getField<DF::String>("score")->value);
              int bScore = std::stoi(b.second.template getField<DF::String>("score")->value);
              return aScore > bScore;
            });

  if (scoreListObject->builder.size() > 10)
    scoreListObject->builder.erase(scoreListObject->builder.begin() + 10, scoreListObject->builder.end());
